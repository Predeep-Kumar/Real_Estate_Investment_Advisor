...
def is_host_reachable(uri: str, timeout: float = 0.8) -> bool:
    try:
...
FALLBACK_DIR_CANDIDATES = [
    os.path.join(".", "reports", "metrics"),
    os.path.join("reports", "metrics"),
    os.path.join(".", "reports", "metrics"),
    os.path.join("reports", "metrics"),
]

FALLBACK_PREFERRED_FILES = {
    "classification": "best_classification_model.json",
    "regression": "best_regression_model.json"
}

FALLBACK_KEYS = ["model_path", "path", "local_path", "joblib", "artifact_uri", "artifact_path", "local_uri", "model_uri", "uri", "file"]

def find_fallback_dir() -> Optional[str]:
    """
    Return the first existing fallback directory from candidates, or try to locate report/metric anywhere under cwd.
    """
    for p in FALLBACK_DIR_CANDIDATES:
        if os.path.isdir(p):
            return p
    # try a more flexible search: any directory named report/metric under cwd
    for root, dirs, files in os.walk(os.getcwd()):
        # check for pattern 'report/metric' within path
        normalized = root.replace("\\", "/")
        if normalized.endswith("/report/metric") or normalized.endswith("/.report/metric"):
            return root
    return None

def find_fallback_json(kind: str) -> Optional[str]:
    """
    Discover a JSON describing a local model for the given kind ('classification' or 'regression').
    Returns path or None.
    """
    kind = kind.lower()
    fb_dir = find_fallback_dir()
    if not fb_dir:
        return None

    # Preferred filename first
    pref = FALLBACK_PREFERRED_FILES.get("classification" if "class" in kind else "regression")
    if pref:
        p = os.path.join(fb_dir, pref)
        if os.path.isfile(p):
            return p

    # Otherwise scan all json files for expected keys
    for fname in os.listdir(fb_dir):
        if not fname.lower().endswith(".json"):
            continue
        p = os.path.join(fb_dir, fname)
        try:
            j = read_json(p)
            if not isinstance(j, dict):
                continue
            # if it contains any of our known keys, accept it
...
def extract_candidate_paths_from_json(cfg: Optional[dict]) -> List[str]:
...
def try_load_local_model_from_path(path: str):
    path = normalize_uri(path)
    # joblib / pickle file
    if isinstance(path, str) and path.lower().endswith((".joblib", ".pkl", ".pickle")) and os.path.exists(path):
        return joblib.load(path)
    # mlflow model dir
    if os.path.isdir(path):
        return mlflow.pyfunc.load_model(path)
    # last resort: mlflow loader (may raise)
    return mlflow.pyfunc.load_model(path)

# ----- Tracking safety -----
def tracking_server_reachable_for_uri(uri: str) -> bool:
    if not isinstance(uri, str) or uri == "":
        return False
    u = uri.lower()
    if os.path.exists(uri) and not (u.startswith("http://") or u.startswith("https://")):
        return True
    if u.startswith("http://") or u.startswith("https://") or u.startswith("models:") or u.startswith("runs:"):
        track = os.environ.get("MLFLOW_TRACKING_URI", DEFAULT_TRACKING_URI)
        if isinstance(track, str) and (track.startswith("http://") or track.startswith("https://")):
            return is_host_reachable(track)
        if isinstance(track, str) and (track.startswith("file://") or os.path.exists(track)):
            return True
        return False
    return False

# ----- Loaders (UI-aware and quiet) -----
def try_load_model_uri_with_fallback(uri: str, kind: str) -> Optional[Any]:
    """
    UI-aware loader used for autoload. It discovers fallback jsons automatically.
    """
    # Try MLflow if safe
    if uri:
        safe = tracking_server_reachable_for_uri(uri)
        if safe:
            try:
                m = load_mlflow_model_cached(uri)
                st.sidebar.success(f"{kind} loaded from URI")
                return m
            except Exception as e:
                st.sidebar.warning(f"{kind} load from URI failed: {e}")
        else:
            st.sidebar.info(f"Skipping MLflow load for {kind} (tracking unreachable); using local fallback.")

    # Discover fallback JSON
    fallback_json = find_fallback_json(kind)
    if fallback_json:
        cfg = read_json(fallback_json)
        if cfg:
            candidates = extract_candidate_paths_from_json(cfg)
            for p in candidates:
                try:
                    if not p:
                        continue
                    loaded = try_load_local_model_from_path(p)
                    st.sidebar.success(f"{kind} loaded from fallback: {p}")
                    return loaded
                except Exception as e:
                    st.sidebar.warning(f"Fallback candidate failed ({p}): {e}")
        else:
            st.sidebar.info(f"Fallback JSON present but unreadable: {fallback_json}")
    else:
        st.sidebar.info(f"No fallback JSON discovered in report/metric for {kind}")

    # Try config/ jsons as final fallback (same extraction)
    cfg_alt = read_json(CLF_JSON if "class" in kind.lower() else REG_JSON)
    if cfg_alt:
        candidates = extract_candidate_paths_from_json(cfg_alt)
        for p in candidates:
            try:
                if not p:
                    continue
                loaded = try_load_local_model_from_path(p)
                st.sidebar.success(f"{kind} loaded from config fallback: {p}")
                return loaded
            except Exception as e:
                st.sidebar.warning(f"Config fallback candidate failed ({p}): {e}")

    st.sidebar.error(f"{kind} could not be loaded from URI or local fallbacks")
    return None

def try_load_model_uri_with_fallback_quiet(uri: str, kind: str):
    """
    Quiet loader for cached metric functions (no Streamlit calls).
    Automatically discovers fallback jsons.
    """
    if uri:
        safe = tracking_server_reachable_for_uri(uri)
        if safe:
            try:
                return load_mlflow_model_cached(uri)
            except Exception:
                pass
        else:
            pass
        ...
        if cfg:
            ...
            for p in candidates:
                if not p:
                    continue
                try:
                    return try_load_local_model_from_path(p)
                except Exception:
                    continue
...
try:
    if isinstance(tracking_uri, str) and tracking_uri.startswith("http") and not is_host_reachable(tracking_uri):
...
# Auto-load models immediately (use UI-aware loader; it will skip MLflow fast if unreachable)
if not st.session_state.get("models_loaded", False):
    st.session_state["model_load_errors"] = []
    st.sidebar.info("Loading selected models (cached)...")
    try:
        st.session_state["reg_model"] = try_load_model_uri_with_fallback(reg_uri, "Regression") if (reg_uri or find_fallback_json("regression")) else None
        st.session_state["clf_model"] = try_load_model_uri_with_fallback(clf_uri, "Classification") if (clf_uri or find_fallback_json("classification")) else None
...
def compute_classification_metrics(uri: str, rows_sig: int, sum_sig: int) -> Dict[str, Any]:
    ...
    # quiet load
    try:
        model_obj = try_load_model_uri_with_fallback_quiet(uri, "Classification")
        if model_obj is None:
            result["error"] = "Model load failed: unable to load model from URI or local fallback"
            return result
    except Exception as e:
...
def compute_regression_metrics(uri: str, rows_sig: int, sum_sig: int) -> Dict[str, Any]:
    ...
    # quiet load
    try:
        model_obj = try_load_model_uri_with_fallback_quiet(uri, "Regression")
        if model_obj is None:
            result["error"] = "Model load failed: unable to load model from URI or local fallback"
            return result
    except Exception as e:
...
# ----- Build comparisons and compute metrics -----
...
if run_clicked:
    ...
    # Right: Investment decision + simplified YES/NO card (fixed colors)
   # Right: Investment decision (single-color donut)
    with right_col:
        ...
        if clf_prob_single is not None:
            try:
                ...
                # Single-color donut (one slice only)
                if decision == "Yes":
                    labels = ["Yes"]
                    values = [1]
                    colors = ["#2ca02c"]   # Green
...
# ---- optional: classifier ROC & confusion after gauges ----
st.markdown("---")
